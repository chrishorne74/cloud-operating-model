...
A [Claude](https://claude.com/claude-code) **skill** (with a matching GitHub Copilot prompt-file
edition) for describing, designing, comparing, scoring, and recommending cloud operating models in a
corporate environment — and producing a phased transition roadmap to move from the current model to a
target one.
...
Vendor framework specifics evolve (Azure CAF renamed operating models in 2025; AWS guidance changes).
When current, citable wording is needed, the skill verifies against live documentation sources
(Microsoft Learn, AWS docs) where available, and attributes named frameworks (Azure CAF, AWS, Gartner,
Team Topologies) accurately. The archetype catalog stands on its own without them.

## Related

A companion skill for the enterprise architecture function lives in its own repo:
[enterprise-architecture-team](https://github.com/chrishorne74/enterprise-architecture-team).
